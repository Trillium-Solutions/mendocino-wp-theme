...
if ( ! function_exists( 'mendocino_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function mendocino_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'mendocino' ),
...
if ( ! function_exists( 'mendocino_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function mendocino_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'mendocino' ) );
		if ( $categories_list && mendocino_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'mendocino' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'mendocino' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'mendocino' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
 ...
 * @return bool
 */
function mendocino_categorized_blog() {
